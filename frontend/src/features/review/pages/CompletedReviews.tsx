...
export function CompletedReviews() {
  ...
  const fetchQueue = useCallback(async () => {
    ...
    try {
      ...
      // Only show assignments that have been completed
      const completedReviews = res.data.filter((r: QueueRow) => !!r.completed_at)
      setRows(completedReviews)
    } catch {
      /* empty */
    } finally {
      setLoading(false)
    ...
    },
    {
      ...
      render: (row) => (
        <span className="inline-flex items-center gap-1 rounded-pill bg-moss/15 px-2 py-0.5 font-mono text-xs font-semibold text-moss">
          ...
          {row.completed_at ? formatDate(row.completed_at) : 'N/A'}
        </span>
      ),
    },
  ]
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-sand bg-white shadow-card">
          <DataTable
            ...
            emptyMessage="You haven't completed any reviews yet."
            pageSize={15}
          />
        </div>
      )}
